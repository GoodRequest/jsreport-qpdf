...
module.exports = (reporter, definition) => {
  ...
  reporter.afterTemplatingEnginesExecutedListeners.add('pdf-qpdf', (req, res) => {
  ...
  reporter.beforeRenderListeners.insert({ after: 'data' }, 'pdf-qpdf', async (req, res) => {
    function pdfFormField (el) {
      ...
      return `<span class='jsreport-pdf-qpdf-form-element jsreport-pdf-qpdf-hidden-element' style='font-family: Helvetica;display: inline-block;vertical-align:middle;text-transform:none;font-size:1.1px;width: ${el.width}px; height: ${el.height}px'>form@@@${value}@@@</span>`
    }
    ...
    function pdfCreatePagesGroup (groupId) {
      ...
      const result = `<span class='jsreport-pdf-qpdf-page-group jsreport-pdf-qpdf-hidden-element' style='font-family: Helvetica;position:absolute;text-transform: none;opacity: 0.01;font-size:1.1px'>group@@@${value}@@@</span>`
    ...
    function pdfAddPageItem (item) {
      ...
      // we use position: absolute to make the element to not participate in flexbox layout
      // (making it not a flexbox child)
      const result = `<span class='jsreport-pdf-qpdf-page-item jsreport-pdf-qpdf-hidden-element' style='font-family: Helvetica;position:absolute;text-transform: none;opacity: 0.01;font-size:1.1px'>item@@@${value}@@@</span>`
  ...
  reporter.afterRenderListeners.insert(0, 'pdf-qpdf', async (req, res) => {
    if (
    ...
    if (
      req.template.pdfPasswordQpdf != null &&
      (
        req.template.pdfPasswordQpdf.password != null ||
        req.template.pdfPasswordQpdf.ownerPassword != null
      )
    ) {
      pdfPassword = req.template.pdfPasswordQpdf
    }
    ...
    if (isPreviewRequest && pdfPassword != null) {
      reporter.logger.debug('Skipping pdf-qpdf password addition, the feature is disabled during preview requests', req)
      pdfPassword = null
    }

    if (isPreviewRequest && pdfSign != null) {
      reporter.logger.debug('Skipping pdf-qpdf signature addition, the feature is disabled during preview requests', req)
      pdfSign = null
    }

    reporter.logger.info('pdf-qpdf is starting pdf processing', req)
    ...
    try {
      ...
      throw reporter.createError('Error while executing pdf-qpdf operations', {
        original: e,
        weak: true
      })
    }

    reporter.logger.info('pdf-qpdf pdf processing was finished', req)
  })
}
