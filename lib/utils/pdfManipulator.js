...
const tempDir = path.join(os.tmpdir(), 'jsreport', 'qpdf')
...
module.exports = (contentBuffer, { pdfMeta, pdfPassword, pdfSign, outlines, removeHiddenMarks } = {}) => {
  ...
  return {
    ...
    async prepend (prependBuffer) {
      const curPDF = path.join(tempDir, v4() + '.pdf')
      const prepPDF = path.join(tempDir, v4() + '.pdf')
    ...
    async postprocess ({ hiddenPageFields }) {
      if (pdfPassword && pdfPassword.password) {

        const curPDF = path.join(tempDir, v4() + '.pdf')
        const finalPDF = path.join(tempDir, v4() + 'finalPass.pdf')
