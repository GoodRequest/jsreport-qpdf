...
module.exports = (contentBuffer, { pdfMeta, pdfPassword, pdfSign, outlines, removeHiddenMarks } = {}, reporter) => {
  ...
  return {
    ...
    async append (appendBuffer) {
      ...
      const curPDF = path.join(reporter.documentStore.options.tempDirectory, v4() + '.pdf')
      const appPDF = path.join(reporter.documentStore.options.tempDirectory, v4() + '.pdf')

      await Promise.all([
        fs.promises.writeFile(curPDF, currentBuffer, { encoding: 'binary' }),
        fs.promises.writeFile(appPDF, appendBuffer, { encoding: 'binary' })
      ])
      const final = path.join(reporter.documentStore.options.tempDirectory, 'jsreport', v4() + '.pdf')
    ...
    async prepend (prependBuffer) {
      const curPDF = path.join(reporter.documentStore.options.tempDirectory, v4() + '.pdf')
      const prepPDF = path.join(reporter.documentStore.options.tempDirectory, v4() + '.pdf')

      await Promise.all([
        fs.promises.writeFile(curPDF, currentBuffer, { encoding: 'binary' }),
        fs.promises.writeFile(appPDF, prependBuffer, { encoding: 'binary' })
      ])
      const final = path.join(reporter.documentStore.options.tempDirectory, v4() + 'pdf')
      await new Promise((resolve, reject) => {
    ...
    async postprocess ({
      ...
      if (pdfPassword && pdfPassword.password) {
        ...
        const curPDF = path.join(reporter.documentStore.options.tempDirectory, v4() + '.pdf')
        const finalPDF = path.join(reporter.documentStore.options.tempDirectory, v4() + 'finalPass.pdf')
