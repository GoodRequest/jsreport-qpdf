...
module.exports = async (inputs, reporter, req, res) => {
  ...
  reporter.logger.debug(`pdf-qpdf detected ${operationsToProcess.length} pdf operation(s) to process`, req)
  ...
  for (const operation of operationsToProcess) {
    ...
    reporter.logger.debug(`pdf-qpdf running pdf operation ${operation.type}`, req)
    if (operation.type === 'append-qpdf') {
      const profilerEvent = reporter.profiler.emit({
        type: 'operationStart',
        subtype: 'pdfQpdfAppend',
        name: 'pdf qpdf append',
        previousOperationId: pdfUtilsProfilerEvent.operationId
      }, req, res)

      await manipulator.append(await runRender(templateDef, { $pdf: { pages: manipulator.parsedPdf.pages } }), reporter)
    ...
    if (operation.type === 'prepend-qpdf') {
      const profilerEvent = reporter.profiler.emit({
        type: 'operationStart',
        subtype: 'pdfQpdfPrepend',
        name: 'pdf qpdf prepend',
        previousOperationId: pdfUtilsProfilerEvent.operationId
      }, req, res)
      await manipulator.prepend(await runRender(templateDef, { $pdf: { pages: manipulator.parsedPdf.pages } }), reporter)
      reporter.profiler.emit({
        type: 'operationEnd',
        operationId: profilerEvent.operationId
      }, req, res)
    ...
    //       reporter.logger.debug(`pdf-qpdf invokes merge with group ${manipulator.parsedPdf.pages[i].group}`, req)
    //     }
  ...
  reporter.logger.debug('pdf-qpdf postproces start', req)
  await manipulator.postprocess({
    hiddenPageFields: req.context.shared.pdfUtilsHiddenPageFields
  })
  reporter.logger.debug('pdf-qpdf postproces end', req)
